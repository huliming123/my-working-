...
def method_scores(data,num1,*args):
    ...
    for c_name ,industry_data in data.groupby('c_name'):
        ...
        new_data['scores']=data_score.sum(axis=1)/3
        # 从大到小排序
    ...
    #  放入excel 里面
    final_data.to_excel('final9_data.xlsx')
    return final_data

if __name__=='__main__':
    with open('input_data', 'rb') as f:
        data = pickle.load(f)
    method_scores(data,20,False,True,True,False,True,True)
